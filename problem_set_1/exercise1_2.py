...
def mlp_gaussian_policy(x, a, hidden_sizes, activation, output_activation, action_space):
    ...
    mu = mlp(x, hidden_sizes=list(hidden_sizes) + [action_dim], activation=activation, output_activation=output_activation)
    log_std = tf.get_variable(name='log_std', initializer=-0.5 * np.ones(action_dim, dtype=np.float32))
    std = tf.exp(log_std)
